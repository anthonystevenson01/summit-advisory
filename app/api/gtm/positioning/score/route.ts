...
const DIMS = [
  { key: "di", name: "Differentiation", weight: 0.25 },
  { key: "sp", name: "Specificity", weight: 0.22 },
  { key: "mf", name: "Market Fit", weight: 0.18 },
  { key: "cl", name: "Clarity", weight: 0.15 },
  { key: "cr", name: "Credibility", weight: 0.12 },
  { key: "er", name: "Emotional Resonance", weight: 0.08 },
] as const;
...
export async function POST(req: NextRequest) {
  ...
  if (inputText.length < 20) {
    return NextResponse.json({ error: "Input text must be at least 20 characters." }, { status: 400 });
  }
  if (inputText.length > 5000) {
    return NextResponse.json({ error: "Input text must be under 5,000 characters." }, { status: 400 });
  }
  ...
  const scoringPrompt = `You are a B2B positioning expert specialising in finite, defined markets. Evaluate positioning statements with rigour.

Score the positioning across these ${DIMS.length} dimensions, each 1–5:
${dimLines}
...
${rubric ? `Use these scoring criteria:\n\n${rubric}\n\n` : ""}Reply with ONLY this JSON format, nothing else: { "scores": { "di": 2, "sp": 3, "mf": 1, "cl": 2, "cr": 1, "er": 3 } }`;
