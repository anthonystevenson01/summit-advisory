...
const DIM_NAMES: Record<string, string> = {
  ...
  ne: "Network / Data Effect",
  br: "Brand & Relationships",
  sd: "Speed & Execution",
  pr: "Price Defensibility",
};
...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `You are a competitive strategy expert focused on B2B SaaS and enterprise software. Reply with only valid JSON, no markdown or extra text.

The moat description has already been scored. Provide detailed reasoning and recommendations.
...
- "reasoning": 2-3 sentences explaining what evidence you found (or didn't find) in the moat description. Quote specific phrases where possible.
...
- "consequence": 1 sentence on what this gap means for competitive positioning
...
${rubric ? rubric : ""}`;
  ...
  try {
    ...
    const stream = anthropic.messages.stream({
      ...
      max_tokens: 2048,
      system: systemPrompt,
      messages: [{ role: "user", content: `Provide detailed reasoning and recommendations for this competitive moat description.\n\n---\n\n${inputText}` }],
    });
