...
const DIMS = [
  { key: "sc", name: "Switching Cost", weight: 0.25 },
  { key: "pd", name: "Product Differentiation", weight: 0.22 },
  { key: "ne", name: "Network / Data Effect", weight: 0.18 },
  { key: "br", name: "Brand & Relationships", weight: 0.15 },
  { key: "sd", name: "Speed & Execution", weight: 0.12 },
  { key: "pr", name: "Price Defensibility", weight: 0.08 },
] as const;
...
export async function POST(req: NextRequest) {
  ...
  const scoringPrompt = `You are a competitive strategy expert focused on B2B SaaS and enterprise software. Evaluate competitive moat descriptions candidly. Most companies overestimate their moat — push back hard.

Score the moat across these ${DIMS.length} dimensions, each 1–5:
${dimLines}
...
${rubric ? `Use these scoring criteria:\n\n${rubric}\n\n` : ""}Reply with ONLY this JSON format, nothing else: { "scores": { "sc": 2, "pd": 3, "ne": 1, "br": 2, "sd": 1, "pr": 3 } }`;
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{ role: "user", content: `Score this competitive moat description. Return JSON only.\n\n---\n\n${inputText}` }],
    });
