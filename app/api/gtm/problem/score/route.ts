...
const DIMS = [
  { key: "pa", name: "Problem Acuity", weight: 0.25 },
  { key: "me", name: "Market Evidence", weight: 0.20 },
  { key: "ug", name: "Urgency", weight: 0.18 },
  { key: "ad", name: "Addressability", weight: 0.15 },
  { key: "cg", name: "Competitive Gap", weight: 0.12 },
  { key: "ms", name: "Monetisation Signal", weight: 0.10 },
] as const;
...
export async function POST(req: NextRequest) {
  ...
  const scoringPrompt = `You are a B2B market validation expert. Evaluate whether a stated market problem is worth building a company around.

Score the problem across these ${DIMS.length} dimensions, each 1–5:
${dimLines}
...
${rubric ? `Use these scoring criteria:\n\n${rubric}\n\n` : ""}Reply with ONLY this JSON format, nothing else: { "scores": { "pa": 2, "me": 3, "ug": 1, "ad": 2, "cg": 1, "ms": 3 } }`;
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [{ role: "user", content: `Score this problem definition. Return JSON only.\n\n<user_input>\n${inputText}\n</user_input>` }],
    });
