...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `You are a B2B market validation expert. Reply with only valid JSON, no markdown or extra text.

The problem definition has already been scored. Provide detailed reasoning and recommendations.
...
- "reasoning": 2-3 sentences explaining what evidence you found (or didn't find) in the problem description. Quote specific phrases where possible.
...
${rubric ? rubric : ""}`;
  ...
  try {
    ...
    const stream = anthropic.messages.stream({
      ...
      max_tokens: 2048,
      system: systemPrompt,
      messages: [{ role: "user", content: `Provide detailed reasoning and recommendations for this problem definition.\n\n---\n\n${inputText}` }],
    });
