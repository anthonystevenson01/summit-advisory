...
export default function PersonaTool({ systemPrompt: _systemPrompt, rubric }: Props) {
  ...
  const handleUnlock = async () => {
    ...
    fetch("/api/gtm/unlock", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: scoreResult.id, tool: "persona", name, email, company }),
    }).catch(() => {});

    try {
      const res = await fetch("/api/gtm/persona/details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                ...
                {!unlocked ? (
                  ...
                  <>
                    ...
                    <div style={{ marginTop: 32, background: BRAND.darkGreen, borderRadius: 10, padding: "28px 24px", textAlign: "center" }}>
                      <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: 18, fontWeight: 600, color: BRAND.white, margin: "0 0 8px" }}>Need help building a stronger persona?</h3>
