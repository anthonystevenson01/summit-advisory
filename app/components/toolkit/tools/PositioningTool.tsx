...
export default function PositioningTool({ systemPrompt: _systemPrompt, rubric }: Props) {
  ...
  const buildInputText = () => {
    const parts: string[] = [];
    if (statement.trim()) parts.push(`Positioning Statement:\n${statement.trim()}`);
  ...
  const handleEvaluate = async () => {
    ...
    try {
      const res = await fetch("/api/gtm/positioning/score", {
        method: "POST",
  ...
  const handleUnlock = async () => {
    ...
    fetch("/api/gtm/unlock", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: scoreResult.id, tool: "positioning", name, email, company }),
    }).catch(() => {});

    try {
      const res = await fetch("/api/gtm/positioning/details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div style={{ minHeight: "100vh", background: BRAND.white, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${BRAND.darkGreen} 0%, ${BRAND.teal} 100%)`, padding: "40px 32px 56px", textAlign: "center" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "inline-block", padding: "4px 14px", borderRadius: 20, background: "rgba(49,154,101,0.25)", border: "1px solid rgba(49,154,101,0.4)", marginBottom: 24 }}>
            <span style={{ color: BRAND.brandGreen, fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>Tool 04 — GTM Toolkit</span>
          </div>
      ...
      <section style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px 16px" }}>
        <div className="gtm-tool-grid" style={{ margin: "0 0 24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ padding: "20px 24px", background: BRAND.white, borderRadius: 10, border: `1px solid ${BRAND.border}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                { dim: "Differentiation", tip: "Your claim is specific enough that it could not be made by any of your top 5 competitors without being obviously false." },
                { dim: "Specificity", tip: "Named customer segment, named outcome, named mechanism — no generic language that could apply to any B2B vendor." },
                { dim: "Market Fit", tip: "The statement addresses real buying criteria that your specific market cares about, not generic benefits." },
                { dim: "Clarity", tip: "A first-time reader understands exactly who you serve, what you do, and why it matters — in under 10 seconds." },
                { dim: "Credibility", tip: "Evidence or proof points are woven in — case study references, metrics, or verifiable claims." },
                { dim: "Emotional Resonance", tip: "It speaks to what the buyer actually cares about, not just features — connecting to real stakes and consequences." },
              ].map((t) => (
        ...
        <p style={{ fontSize: 15, color: BRAND.dark, lineHeight: 1.7, margin: "0 0 6px" }}>
          Paste your positioning statement below. Use the format: <em>&ldquo;We help [customer] achieve [outcome] by [mechanism], unlike [alternative].&rdquo;</em>
        </p>
      </section>
      ...
      <section style={{ maxWidth: 960, margin: "0 auto", padding: "8px 24px" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: BRAND.darkGreen, display: "block", marginBottom: 6, letterSpacing: "0.02em" }}>
            Positioning Statement <span style={{ color: BRAND.red }}>*</span>
          </label>
          <textarea
            value={statement}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 4 }}>
            <span style={{ fontSize: 12, color: statement.length > MAX_STATEMENT * 0.9 ? BRAND.amber : BRAND.mid }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: BRAND.darkGreen, display: "block", marginBottom: 6, letterSpacing: "0.02em" }}>
            Additional Context <span style={{ color: BRAND.mid, fontWeight: 400 }}>(optional)</span>
          </label>
          <input
            type="text"
            value={context}
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          {!scoreRevealed && (
            <button
              ...
              disabled={buildInputText().length < 20 || phase === "scoring"}
              style={{
                padding: "12px 28px",
                borderRadius: 6,
                border: "none",
                background: buildInputText().length < 20 ? BRAND.border : BRAND.brandGreen,
                color: BRAND.white,
                fontFamily: "'Oswald', sans-serif",
                fontWeight: 600,
                fontSize: 15,
                letterSpacing: "0.04em",
                cursor: buildInputText().length < 20 ? "not-allowed" : "pointer",
                transition: "all 0.2s",
                opacity: phase === "scoring" ? 0.7 : 1,
              }}
            >
              {phase === "scoring" ? "Scoring…" : "Grade My Positioning"}
            </button>
          )}
        </div>
                ...
                {!unlocked ? (
                  ...
                  <>
                    ...
                    <div style={{ marginTop: 32, background: BRAND.darkGreen, borderRadius: 10, padding: "28px 24px", textAlign: "center" }}>
                      <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: 18, fontWeight: 600, color: BRAND.white, margin: "0 0 8px" }}>Need help sharpening your positioning?</h3>
                      <p style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", margin: "0 0 18px" }}>
                        Summit works alongside small teams selling to enterprise — refining positioning, building account strategies, and closing the gap between aspiration and pipeline reality.
                      </p>
                      <a
